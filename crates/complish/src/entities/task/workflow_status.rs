...
mod test {
  ...
  mod from_sql {
    ...
    fn it_parses() {
      ...
      let blocked: WorkflowStatus = statement.query_row(["blocked"], |row| row.get(0)).unwrap();
  ...
  mod from_str {
    ...
    fn it_parses() {
      assert_eq!(
        WorkflowStatus::from_str("blocked").unwrap(),
  ...
  mod to_sql {
    ...
    fn it_serializes() {
      ...
      let blocked: WorkflowStatus = statement
        .query_row([WorkflowStatus::Blocked], |row| row.get(0))
        .unwrap();
