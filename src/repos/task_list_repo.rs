...
impl TaskListRepo {
  ...
  pub fn add_task(&self, task_list: &TaskList, mut task: Task) -> Result<TaskList> {
    ...
    let old_list_name = task.list_name.clone();
    let old_file_path = self.task_path.join(&old_list_name).join(format!("{}.md", task.display_id));
    ...
    index.move_task(task.id, &old_list_name, &task_list.name, &task.display_id)?;
